...
final class LayeredTower implements Cloneable {
    ...
    public void updateMonsterPosition(final int move, final int xLoc,
        ...
        try {
            final AbstractMazeObject there = this.getMazeCell(xLoc + dirMove[0],
                    yLoc + dirMove[1], zLoc, MazeConstants.LAYER_OBJECT);
            final AbstractMazeObject ground = this.getMazeCell(
    ...
    private void generateOneMonster() {
        ...
        AbstractMazeObject currObj = this.getMazeCell(randomRow, randomColumn,
    ...
    private boolean isSquareVisibleLOS(final int x1, final int y1, final int x2,
        ...
        do {
            ...
            try {
                final AbstractMazeObject obj = this.getMazeCell(fx1, fy1, zLoc,
                        MazeConstants.LAYER_OBJECT);
    ...
    public void tickTimers(final int floor) {
        ...
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                final AbstractMazeObject mo = this.getMazeCell(y, x, floor,
                        MazeConstants.LAYER_OBJECT);
                if (mo != null) {
                    mo.tickTimer(y, x);
                }
    ...
    public void fullScanButton(final int l) {
        ...
        for (u = 0; u < this.getColumns(); u++) {
            for (v = 0; v < this.getRows(); v++) {
                final AbstractMazeObject testObj = this.getMazeCell(v, u, z, l);
